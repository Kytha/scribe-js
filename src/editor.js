...
import { onTab } from "./util/onTab";
import ImageButton from "./components/sides/image";
import GraphButton from "./components/sides/graph";
...
class MediumDraftEditor extends React.Component {
  static propTypes = {
    ...
    graphCaptionPlaceholder: PropTypes.string,
  ...
  static defaultProps = {
    ...
    graphCaptionPlaceholder: "Add graph caption",
    continuousBlocks: [
      Block.UNSTYLED,
      Block.BLOCKQUOTE,
      Block.OL,
  ...
  onTab(e) {
    ...
    if (blockType === Block.CODE) {
      this.onChange(onTab(e, editorState));
      return HANDLED;
    }
  ...
  */
  ...
  handlePastedText = (text, html, es) => {
    ...
    if (
      currentBlock.getType() === Block.IMAGE ||
      currentBlock.getType() === Block.CODE
    ) {
      const { editorState } = this.props;
      const content = editorState.getCurrentContent();
      this.onChange(
        EditorState.push(
          editorState,
          Modifier.replaceText(content, editorState.getSelection(), text)
        )
      );
      return HANDLED;
    }
