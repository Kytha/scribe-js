...
export const htmlToStyle = (nodeName, node, currentStyle) => {
  switch (nodeName) {
    ...
    case "span":
      ...
      if (node.className === `md-inline-${Inline.EQUATION.toLowerCase()}`) {
        return currentStyle.add(Inline.EQUATION);
      }
      break;
    case "code":
...
export const htmlToBlock = (nodeName, node) => {
  if (nodeName === "h1") {
    ...
    } else if (node.className === `md-block-${Block.GRAPH.toLowerCase()}`) {
      const graphNode = node.querySelector("span");
      return {
        type: Block.GRAPH,
        data: { id: graphNode.id, config: graphNode.dataset.config },
      };
    ...
    ["div", "pre"].includes(nodeName) &&
    node.className === "md-block-code-container"
  ) {
    return {
      type: Block.CODE,
      data: {},
